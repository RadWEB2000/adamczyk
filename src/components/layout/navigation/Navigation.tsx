...
export default function Navigation(){
    ...
    return (
        <nav className="bg-transparent select-none fixed top-0 left-0 w-full z-99999 text-neutral-400 px-4 grid data-[expand=false]:grid-rows-[4rem_0] data-[expand=true]:grid-rows-[4rem_calc(100vh-4rem)] overflow-hidden grid-cols-3 data-[expand=true]:bg-brand-950 transition-all duration-200 ease-in-out sm:data-[expand=false]:grid-rows-[6rem_0] sm:data-[expand=true]:grid-rows-[6rem_calc(100vh-6rem)] lg:grid-cols-[2fr_4fr] h-fit lg:data-[expand=true]:grid-rows-[5.5rem] lg:data-[expand=false]:grid-rows-[5.5rem] lg:items-center lg:px-10 lg:overflow-visible xl4:px-8.75 data-[scrolled=true]:bg-brand-950!" data-expand={isExpandMenu} data-scrolled={isScrolled}>
            <Brand/>
            <Menu/>
            <div
                className="items-center justify-end flex  col-start-3 col-end-4 row-start-1 lg:hidden"
            >
                <MenuButton/>
            </div>
        </nav>
