import type { T_RegularHero } from "@/ts/ui.types"
...
export default function RegularHero({breadcrumbs,subtitle,title,content,button}:T_RegularHero){
    return (
        <header className="bg-brand-950 min-h-[54vh] bg-[linear-gradient(to_right,color-mix(in_srgb,var(--color-brand-200),transparent_86%)_1px,transparent_1px)] bg-size-[105px_150px]  text-neutral-200 grid grid-cols-1 *:row-start-1 z-0 relative landscape:sm4:max-md2:grid-cols-5">
            <section
                className="col-start-1   z-1 flex flex-col items-start justify-end pb-10 px-9 space-y-5 landscape:sm4:max-md2:col-end-5 bg-sky-500/0 landscape:sm4:max-md2:space-y-2 md:col-end-10 md:px-13 md:pb-15  2xl:px-25  2xl:pb-[10vh]  2xl:space-y-8 pt-25 xl4:grid grid-cols-[4fr_3fr]"
            >
                <div
                ...
                </div>
                {
                    content &&
                    <p
                        className="text-sm bg-green-600/0 leading-relaxed text-balance landscape:sm4:max-md2:text-xs md:text-base  lg:text-lg xl5:text-lg-plus  mt-auto mb-0 2xl5:text-2xl w-full"
